def fine_tuning(tokenizer,arg, train_bool,img_path ):    
    import warnings
    ...
    path = arg['train_img_path']
    root = arg['train_img_path']
    path_description = arg['path_description']

    MAX_SEQ_LEN = arg['MAX_SEQ_LEN'] #최대 글자 수 
    LR = arg['LR'] # Learning Rate
    EPOCHS = arg['EPOCHS']
    IMAGE_SIZE = arg['IMAGE_SIZE'] #256x256
    ...
    path = 'ImageSource/images'
    data = np.zeros((10001, 2), dtype=np.object)
    i = 0
    for line in open('ImageDiscription/test.txt', 'r', encoding='UTF8'):
        data[i, :] = line.replace('\n', "").split('|')
        i += 1
    ...
    # path_description = '/workspace/IMAGE/test.txt'
